...
class Solution:
    def isSumEqual(self, firstWord: str, secondWord: str, targetWord: str) -> bool:
        dd = {chr(x + 97): str(x) for x in range(26)}

        f_s = self.sum_word(firstWord, dd)
        s_s = self.sum_word(secondWord, dd)
        t_s = self.sum_word(targetWord, dd)

        return f_s + s_s == t_s

    def sum_word(self, word, dd):
        return int(''.join(dd[x] for x in word))
