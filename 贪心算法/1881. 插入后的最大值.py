...
class Solution:
    def maxValue(self, n: str, x: int) -> str:
        ...
        if '-' in n:
            res = self.negative_number_insert(list(n)[1:], str(x))
            return '-' + ''.join(res)

        res = self.positive_number_insert(list(n), str(x))
        return ''.join(res)

    def positive_number_insert(self, nn: list, xx: str):
        """
        正数
        """
    ...
    def negative_number_insert(self, nn: list, xx: str):
        """
        负数
        """
        for i, x in enumerate(nn):
